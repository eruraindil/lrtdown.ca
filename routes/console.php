...
Artisan::command('twitter:get', function () {
    ...
    $filteredTweets = preg_grep('/((delay|close)|(eastbound.*westbound|westbound.*eastbound)|(eastbound|westbound)\s?(platform)?\sonly|r1.*between|allow extra travel time|switch issue|replacement bus|door fault|stopped train|must use platform|wait times of up to \d\d+ (minutes|mins))/miU', $tweets);
